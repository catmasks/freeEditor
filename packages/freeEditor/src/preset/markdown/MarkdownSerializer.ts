...
import { MarkdownSerializer as PMMarkdownSerializer } from "prosemirror-markdown";
...
export function createMarkdownSerializer(
  ...
  const defaultNodes: Record<string, (...args: any[]) => void> = {
    ...
    image(state: any, node: any) {
      ...
      if (node.type.spec.inline === false) state.closeBlock(node);
    },
    ...
    tableCell(state: any, node: any) { state.renderInline(node); },
    tableHeader(state: any, node: any) { state.renderInline(node); },
    text(state: any, node: any) { state.text(node.text ?? ""); },
  };
  ...
  const defaultMarks: Record<string, MarkdownMarkSerializer> = {
    ...
    inlineCode: { open: "`", close: "`", mixable: false, expelEnclosingWhitespace: false },
    link: {
      open: "[",
  ...
  const mergedNodes: Record<string, (...args: any[]) => void> = { ...defaultNodes };
  ...
  return new PMMarkdownSerializer(mergedNodes, { ...defaultMarks, ...extraMarks });
}